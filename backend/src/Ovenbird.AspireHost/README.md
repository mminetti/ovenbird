...
1. Set `Ovenbird.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../Ovenbird.Infrastructure/Ovenbird.Infrastructure.csproj -s Ovenbird.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../Ovenbird.Infrastructure/Ovenbird.Infrastructure.csproj -s Ovenbird.Web.csproj
```
